...
class regularMode extends StatefulWidget {
  regularMode({required this.updateFsFunc, required this.email});
  final Function updateFsFunc;
  String email;

  @override
  State<regularMode> createState() => regularModeStates();
}

class regularModeStates extends State<regularMode> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(child:SnappingSheet(
      ...
      grabbing:GestureDetector(
        ...
        child : Column(children:[Container(height:45,color: Colors.grey,child: Row(
          children:[Row(mainAxisAlignment: MainAxisAlignment.start,children: [SizedBox(width: 10,),
            Text('Welcome back, '+ widget.email),],), Padding(padding: EdgeInsets.only(left:200),
            child: Icon( Icons.keyboard_arrow_up, color: Colors.black,),)],),),
          Container(height:150,color: Colors.white,child: Row(
            children:[Row(mainAxisAlignment: MainAxisAlignment.start,children: [SizedBox(width: 10,),
              Text('Welcome back, '+ widget.email),],), Padding(padding: EdgeInsets.only(left:200),
              child: Icon( Icons.keyboard_arrow_up, color: Colors.black,),)],),),]),),
